...
- **Sensor Analyzer** — modeless window for reading a *device* recording (a phone, an edge box) as an instrument rather than as a pipeline: the video and every recorded sensor stream hang off a single playhead, so a change in one is read against the others at the instant it happened. Shows where the device was pointing and the ground track it followed, draws the camera's field of view from the figure the device itself recorded (falling back to lens + sensor geometry, and drawing no cone at all when neither is known), and overlays what the instruments held at the playhead onto the frame. Read-only and engine-free — it owns its own session reader, opens with the pipeline stopped, and cannot disturb a run. Reached from Tools → Sensor Analyzer, or the **Analyze** button on a device recording in the Recordings view. **Not tier-gated**: a phone records on Community, and the analyzer is what makes that recording worth looking at.
